...
export const propertySlice = createSlice({
  ...
  reducers: {
    single_property: (state, action: PayloadAction<number>) => {
      state.property =
        state.properties.find((p) => Number(p.id) === Number(action.payload)) ||
        {};
    },
  },
});
...
// Selectors
export const selectProperties = (state: { properties: PropertyState }) => {
  console.log('state', state);
  return state?.properties?.properties;
};
// console.log('');

export const selectProperty = (state: { properties: PropertyState }) =>
